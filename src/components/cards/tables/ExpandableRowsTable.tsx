      ...
      <div className="flex-1 overflow-auto" style={{ scrollbarWidth: "thin" }}>
        <table className="w-full text-[13px]" style={{ borderCollapse: "collapse" }}>
          ...
          <tbody>
            {filtered.map(u => (
              <Fragment key={u.id}>
                <tr onClick={() => setExpanded(expanded === u.id ? null : u.id)} className="cursor-pointer" style={{ borderBottom: `1px solid ${t.border}` }} onMouseEnter={e => e.currentTarget.style.background = t.rowHover} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  ...
                  <td className="px-3 py-2.5"><div className="flex items-center gap-2"><img loading="lazy" src={u.avatar} alt="" className="h-7 w-7 rounded-full object-cover" /><span className="font-semibold" style={{ color: t.textPrimary }}>{u.name}</span></div></td>
