          ...
          <tbody>
            {filtered.map(u => (
              <tr key={u.id} style={{ borderBottom: `1px solid ${t.border}` }} onMouseEnter={e => e.currentTarget.style.background = t.rowHover} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                <td className="px-3 py-1.5"><div className="flex items-center gap-2"><img loading="lazy" src={u.avatar} alt="" className="h-6 w-6 rounded-full object-cover" /><span className="font-semibold" style={{ color: t.textPrimary }}>{u.name}</span></div></td>
