        ...
        <div className="space-y-3 md:hidden">
          {filtered.map((u, i) => (
            <motion.div key={u.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="rounded-xl border p-3" style={{ borderColor: t.border, background: t.panelBg }}>
              <div className="flex items-center gap-3 mb-2"><img loading="lazy" src={u.avatar} alt="" className="h-10 w-10 rounded-full object-cover" /><div><p className="text-[13px] font-bold" style={{ color: t.textPrimary }}>{u.name}</p><p className="text-[10px] font-mono" style={{ color: t.textMuted }}>{u.email}</p></div></div>
