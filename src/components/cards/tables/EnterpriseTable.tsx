          ...
          <tbody>
            {filtered.map((u, i) => (
              <tr key={u.id} style={{ borderBottom: `1px solid ${t.border}`, background: i % 2 === 0 ? "transparent" : t.stripeBg }} onMouseEnter={e => e.currentTarget.style.background = t.rowHover} onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? "transparent" : t.stripeBg}>
                <td className="px-3 py-2.5" style={{ position: "sticky", left: 0, zIndex: 5, background: i % 2 === 0 ? t.bg : t.stripeBg, minWidth: 180 }}><div className="flex items-center gap-2"><img loading="lazy" src={u.avatar} alt="" className="h-7 w-7 rounded-full object-cover" /><span className="font-semibold" style={{ color: t.textPrimary }}>{u.name}</span></div></td>
