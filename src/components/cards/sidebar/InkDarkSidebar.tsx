...
export function InkDarkSidebar() {
  ...
  const textSecondary = isDark ? "#525252" : "#737373";
  const textMuted = isDark ? "#404040" : "#a3a3a3";
  const sectionLabel = isDark ? "#262626" : "#d4d4d4";
    ...
    <div className="flex h-full min-h-full w-full overflow-hidden" style={{ background: bg }}>
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center gap-2.5 px-4" style={{ justifyContent: collapsed ? "center" : "flex-start" }}>
          ...
          <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="ml-auto flex items-center gap-1">
            ...
            <button onClick={() => setCollapsed(true)} className="flex h-7 w-7 items-center justify-center rounded-lg transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted }}>
        ...
        <div className="shrink-0 px-3 pb-2">
          {!collapsed ? (
            ...
            <button className="flex h-8 w-full items-center justify-center rounded-lg border transition" style={{ background: inputBg, borderColor: inputBorder, color: textMuted }}><Search className="h-3.5 w-3.5" /></button>
          )}
        </div>
            ...
            <div key={section} className="mb-3">
              ...
              <div className="space-y-0.5">
                {items.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => setActive(item.id)} className="group relative flex w-full items-center gap-3 rounded-lg px-2 py-2 text-[12px] font-medium outline-none transition"
        ...
        <div className="shrink-0 p-2" style={{ borderTop: `1px solid ${border}` }}>
          <div className="flex items-center gap-2.5 rounded-lg transition" style={{ justifyContent: collapsed ? "center" : "flex-start", padding: collapsed ? "0" : "0.25rem" }}>
            ...
            <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0, x: -3 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -3 }} transition={{ duration: 0.12 }} className="flex min-w-0 flex-1 items-center justify-between"><p className="truncate text-[11px] font-medium" style={{ color: textSecondary }}>Alex Morgan</p><button className="transition hover:text-rose-500" style={{ color: textMuted }}><LogOut className="h-3.5 w-3.5" /></button></motion.div>}</AnimatePresence>
          </div>
        </div>
      </motion.aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: bg }}>
        <header className="flex h-14 shrink-0 items-center gap-4 px-6" style={{ borderBottom: `1px solid ${border}` }}>
          ...
          <button className="relative flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted }}><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full" style={{ background: accentColor }} /></button>
        </header>
