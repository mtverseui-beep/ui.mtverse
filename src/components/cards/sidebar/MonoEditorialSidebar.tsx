...
export function MonoEditorialSidebar() {
  ...
  return (
    <div className="flex h-full min-h-full w-full overflow-hidden" style={{ background: bg, fontFamily: "Georgia, 'Times New Roman', serif" }}>
      <motion.aside
      ...
      >
        ...
        <div className="flex h-16 shrink-0 items-center gap-2.5 px-4" style={{ borderBottom: `2px solid ${border}`, justifyContent: collapsed ? "center" : "flex-start" }}>
          ...
          <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="ml-auto flex items-center gap-1">
            <button onClick={toggle} className="flex h-7 w-7 items-center justify-center rounded-none transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted, border: `1px solid ${inputBorder}` }}>
              {isDark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
            </button>
            <button onClick={() => setCollapsed(true)} className="flex h-7 w-7 items-center justify-center rounded-none transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted, border: `1px solid ${inputBorder}` }}><ChevronLeft className="h-3.5 w-3.5" /></button>
          </motion.div>}</AnimatePresence>
        </div>
        ...
        <div className="shrink-0 px-4 py-3">
          {!collapsed ? (
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search the archives..."
            ...
            <button className="flex h-8 w-full items-center justify-center rounded-none border transition" style={{ background: inputBg, borderColor: inputBorder, color: textMuted }}><Search className="h-3.5 w-3.5" /></button>
          )}
        </div>
            ...
            <div key={section} className="mb-4">
              ...
              <div className="space-y-0.5">
                {items.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => setActive(item.id)} className="group relative flex w-full items-center gap-2.5 rounded-none px-3 py-2.5 text-[13px] font-bold outline-none transition"
        ...
        {collapsed && <div className="shrink-0 flex flex-col items-center gap-2 px-2 pb-3"><button onClick={toggle} className="flex h-7 w-7 items-center justify-center rounded-none transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted, border: `1px solid ${inputBorder}` }}>{isDark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}</button><button onClick={() => setCollapsed(false)} className="flex h-8 w-full items-center justify-center rounded-none border transition" style={{ background: inputBg, borderColor: inputBorder, color: textMuted }}><ChevronLeft className="h-3.5 w-3.5 rotate-180" /></button></div>}
        ...
        <div className="shrink-0 p-3" style={{ borderTop: `2px solid ${border}` }}>
          <div className="flex items-center gap-2.5 rounded-none transition" style={{ justifyContent: collapsed ? "center" : "flex-start", padding: collapsed ? "0" : "0.5rem" }}>
            ...
            <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0, x: -3 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -3 }} transition={{ duration: 0.12 }} className="flex min-w-0 flex-1 items-center justify-between">
              ...
              <button className="transition hover:text-rose-600" style={{ color: textMuted }}><LogOut className="h-3.5 w-3.5" /></button>
            </motion.div>}</AnimatePresence>
          </div>
        </div>
      </motion.aside>
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: bg }}>
        <header className="flex h-16 shrink-0 items-center gap-4 px-6" style={{ borderBottom: `2px solid ${border}` }}>
          ...
          <button className="flex h-8 w-8 items-center justify-center rounded-none transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted, border: `1px solid ${inputBorder}` }}><Plus className="h-4 w-4" /></button>
          <button className="relative flex h-8 w-8 items-center justify-center rounded-none transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted, border: `1px solid ${inputBorder}` }}><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full" style={{ background: textPrimary }} /></button>
        </header>
