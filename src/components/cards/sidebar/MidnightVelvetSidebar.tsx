...
export function MidnightVelvetSidebar() {
  ...
  // Velvet palette — stays rich in both modes (dark luxurious)
  const bg = isDark ? "#0f0820" : "#1a0f2e";
  const sidebarBg = isDark ? "#150a26" : "#1f1235";
  const border = isDark ? "rgba(245,158,11,0.12)" : "rgba(245,158,11,0.18)";
  const textPrimary = "#fdf2f8";
  const textSecondary = "rgba(253,242,248,0.65)";
  const textMuted = "rgba(253,242,248,0.40)";
  const sectionLabel = "#f59e0b";
  const inputBg = isDark ? "rgba(245,158,11,0.06)" : "rgba(245,158,11,0.08)";
  const inputBorder = "rgba(245,158,11,0.18)";
  const hoverBg = "rgba(245,158,11,0.08)";
  const goldStart = "#f59e0b";
  const goldEnd = "#d97706";
    ...
    <div className="flex h-full min-h-full w-full overflow-hidden" style={{ background: bg }}>
      ...
      >
        ...
        <div className="flex h-16 shrink-0 items-center gap-3 px-5" style={{ borderBottom: `1px solid ${border}`, justifyContent: collapsed ? "center" : "flex-start" }}>
          ...
          <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="ml-auto flex items-center gap-1">
            <button onClick={toggle} className="flex h-7 w-7 items-center justify-center rounded-lg transition hover:bg-white/5" style={{ color: textMuted }}>
              {isDark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
            </button>
            <button onClick={() => setCollapsed(true)} className="flex h-7 w-7 items-center justify-center rounded-lg transition hover:bg-white/5" style={{ color: textMuted }}><ChevronLeft className="h-3.5 w-3.5" /></button>
          </motion.div>}</AnimatePresence>
        </div>
        ...
        <div className="shrink-0 px-4 py-3">
          {!collapsed ? (
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search the vault..."
            ...
            <div key={section} className="mb-4">
              ...
              <div className="space-y-1">
                {items.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => setActive(item.id)} className="group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[12.5px] font-medium outline-none transition"
                      style={{
      ...
      <div className="relative z-10 flex min-w-0 flex-1 flex-col" style={{ background: bg }}>
        <header className="flex h-16 shrink-0 items-center gap-4 px-6" style={{ borderBottom: `1px solid ${border}` }}>
          ...
          <button className="flex h-9 w-9 items-center justify-center rounded-xl transition hover:bg-white/5" style={{ color: textMuted }}><Plus className="h-4 w-4" /></button>
          <button className="relative flex h-9 w-9 items-center justify-center rounded-xl transition hover:bg-white/5" style={{ color: textMuted }}><Bell className="h-4 w-4" /><span className="absolute right-2 top-2 h-2 w-2 rounded-full" style={{ background: goldStart }} /></button>
        </header>
