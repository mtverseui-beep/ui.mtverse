    ...
    <div ref={containerRef} data-theme={isDark ? "dark" : "light"} className={sidebarRootClassName(isDark, "flex h-full min-h-full w-full overflow-hidden font-mono")} style={{ background: bg }}>
        ...
        <div className="relative z-10 shrink-0 px-3 py-2">
          {!collapsed ? (
            <div className="relative">
              ...
              <input ref={searchInputRef} aria-label="Search navigation" value={search} onChange={e => setSearch(e.target.value)} placeholder="grep..."
            ...
            <div key={section} className="mb-3">
              ...
              <div className="space-y-0.5">
                {items.map(item => {
                  ...
                  return (
                    <button key={item.id} aria-label={item.label} aria-current={isActive ? "page" : undefined} onClick={() => setActive(item.id)} className="group relative flex w-full items-center gap-2.5 rounded px-2 py-2 text-[12px] font-medium outline-none transition"
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: bg }}>
        <header className="flex h-14 shrink-0 items-center gap-4 px-6" style={{ borderBottom: `1px solid ${border}` }}>
          <h1 className="text-[14px] font-bold" style={{ color: textPrimary }}>{NAV.find(i => i.id === active)?.label}<span className="ml-1 inline-block h-3 w-2 animate-pulse motion-reduce:animate-none" style={{ background: accent }} /></h1>
          ...
          <button aria-label="Notifications" className="relative flex h-8 w-8 items-center justify-center rounded transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted }}><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full" style={{ background: accent, boxShadow: isDark ? `0 0 6px ${accent}` : "none" }} /></button>
        </header>
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded" style={{ background: hoverBg, border: `1px solid ${border}` }}><Cpu className="h-7 w-7" strokeWidth={1.5} style={{ color: textMuted }} /></div><p className="text-[13px] font-medium" style={{ color: textMuted }}>system: ready_<span className="animate-pulse motion-reduce:animate-none">|</span></p></div>
        </div>
      </div>
    </div>
  );
