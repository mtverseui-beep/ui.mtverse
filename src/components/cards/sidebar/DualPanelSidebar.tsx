    ...
    <div ref={containerRef} data-theme={isDark ? "dark" : "light"} className={sidebarRootClassName(isDark, "flex h-full min-h-full w-full overflow-hidden")} style={{ background: appBg }}>
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center gap-2.5 px-4" style={{ borderBottom: `1px solid ${headerBorder}`, justifyContent: collapsed ? "center" : "flex-start" }}>
          ...
          <AnimatePresence>{!collapsed && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="ml-auto flex items-center gap-1">
            ...
            <motion.button aria-label="Collapse sidebar" aria-expanded={!collapsed} onClick={() => setCollapsed(true)} className="flex h-7 w-7 items-center justify-center rounded-lg transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted }}><ChevronLeft className="h-4 w-4" /></motion.button>
        ...
        <div className="shrink-0 p-3">
          {!collapsed ? (
            ...
            <button onClick={expandAndFocusSearch} aria-label="Expand sidebar and search" className="flex h-9 w-full items-center justify-center rounded-lg border transition" style={{ background: inputBg, borderColor: inputBorder, color: textMuted }}><Search className="h-4 w-4" /></button>
          )}
        </div>
          ...
          {GROUPS.map(group => {
            ...
            return (
              <div key={group.id} className="mb-1">
                {/* Parent disclosure is only interactive while its controlled panel exists. */}
                {!collapsed ? (
                  <button aria-label={group.label} aria-expanded={isExpanded} aria-controls={isExpanded ? `dual-group-${group.id}` : undefined} onClick={() => toggleGroup(group.id)} className={`flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[12px] font-bold uppercase tracking-wider transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${isDark ? "hover:bg-white/5" : "hover:bg-black/5"}`} style={{ color: sectionLabel }}>
                    <GroupIcon className="h-4 w-4 shrink-0" strokeWidth={2} style={{ color: textMuted }} />
                    <span className="flex-1 text-left">{group.label}</span>
                    <motion.span animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: reducedMotion ? 0 : 0.2 }}><ChevronDown className="h-3.5 w-3.5" style={{ color: textMuted }} /></motion.span>
                  </button>
                ) : (
                  <div className="flex h-9 items-center justify-center" title={group.label}>
                    <GroupIcon aria-hidden="true" className="h-4 w-4" strokeWidth={2} style={{ color: textMuted }} />
                ...
                <AnimatePresence initial={false}>
                  {isExpanded && !collapsed && (
                    <motion.div id={`dual-group-${group.id}`} initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.25, ease: EASE }} className="overflow-hidden">
                      <div className="ml-4 pl-2 pt-1" style={{ borderLeft: `1px solid ${childBorder}` }}>
                        {filteredItems.map(item => {
                          const isActive = active === item.id;
                          const Icon = item.icon;
                          return (
                            <button key={item.id} aria-label={item.label} aria-current={isActive ? "page" : undefined} onClick={() => setActive(item.id)} className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[12px] font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" style={{ color: isActive ? activeColor : textSecondary, background: isActive ? activeBg : "transparent" }}>
                ...
                {collapsed && (
                  <div className="flex flex-col items-center gap-1 py-1">
                    {filteredItems.map(item => {
                      const isActive = active === item.id;
                      const Icon = item.icon;
                      return <button key={item.id} aria-label={item.label} aria-current={isActive ? "page" : undefined} onClick={() => setActive(item.id)} className="h-7 w-7 rounded-lg transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" style={{ background: isActive ? activeBg : "transparent" }}><Icon className="h-3.5 w-3.5 mx-auto" strokeWidth={2} style={{ color: isActive ? activeColor : textMuted }} /></button>;
                    })}
                  </div>
                )}
              </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col" style={{ background: appBg }}>
        <header className="flex h-14 shrink-0 items-center gap-4 px-6" style={{ borderBottom: `1px solid ${mainBorder}`, background: sidebarBg }}>
          ...
          <button aria-label="Notifications" className="relative flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-black/5 dark:hover:bg-white/5" style={{ color: textMuted }}><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-rose-500" /></button>
        </header>
